      ...
      try {
        ...
        if (config.useTraefik && config.domain && config.subdomain) {
          ...
          if (config.isDatabase) {
            ...
            // For PostgreSQL: Mount TLS certificates and configure SSL
            if (dockerImage.includes('postgres')) {
              try {
                // Generate certificates for this database
                const certPaths = await this.certificateService.generateCertificate(
                  config.name,
                  fullDomain
                );
                
                // Create init script directory for PostgreSQL SSL configuration
                const initScriptDir = path.join(process.cwd(), 'init-scripts', config.name);
                if (!fs.existsSync(initScriptDir)) {
                  fs.mkdirSync(initScriptDir, { recursive: true, mode: 0o755 });
                }
                
                // Create custom postgresql.conf with SSL enabled
                // This approach works for both new and existing databases
                const postgresqlConfPath = path.join(initScriptDir, 'postgresql-ssl.conf');
                const postgresqlConf = `# PostgreSQL SSL Configuration
# Auto-generated by Grumpy Wombat Daemon
# This file is included in the main postgresql.conf
...
# SSL protocols (secure defaults)
ssl_min_protocol_version = 'TLSv1.2'
ssl_ciphers = 'HIGH:MEDIUM:+3DES:!aNULL'
`;
                
                fs.writeFileSync(postgresqlConfPath, postgresqlConf, { mode: 0o644 });
                
                // Create init script that configures SSL (runs on first init)
                // This script copies certificates and configures PostgreSQL for SSL
                const initScriptPath = path.join(initScriptDir, '01-configure-ssl.sh');
...
# Copy certificates to data directory (PostgreSQL reads from data dir by default)
mkdir -p /var/lib/postgresql/data
cp /var/lib/postgresql/certs/server.crt /var/lib/postgresql/data/server.crt 2>/dev/null || true
cp /var/lib/postgresql/certs/server.key /var/lib/postgresql/data/server.key 2>/dev/null || true
cp /var/lib/postgresql/certs/ca.crt /var/lib/postgresql/data/ca.crt 2>/dev/null || true

# Set proper permissions (PostgreSQL requires key to be 600 and owned by postgres)
chmod 600 /var/lib/postgresql/data/server.key 2>/dev/null || true
chown postgres:postgres /var/lib/postgresql/data/server.* 2>/dev/null || true
...
# Set certificate paths in postgresql.conf
...
`;
                ...
                // Mount certificates and config files into container
                const certMounts = [
                  `${certPaths.certPath}:/var/lib/postgresql/certs/server.crt:ro`,
                  `${certPaths.keyPath}:/var/lib/postgresql/certs/server.key:ro`,
                  `${certPaths.caPath}:/var/lib/postgresql/certs/ca.crt:ro`,
                  `${postgresqlConfPath}:/var/lib/postgresql/certs/postgresql-ssl.conf:ro`,
                  `${initScriptPath}:/docker-entrypoint-initdb.d/01-configure-ssl.sh:ro`
                ];
                
                // Add certificate mounts to existing binds
                if (!containerConfig.HostConfig.Binds) {
                  containerConfig.HostConfig.Binds = [];
                }
                containerConfig.HostConfig.Binds.push(...certMounts);
                ...
                }, 'Mounted TLS certificates and SSL init script for PostgreSQL container');
              } catch (error) {
                logger.warn({ 
                  instanceName: config.name, 
                  error: this.getErrorMessage(error) 
                }, 'Failed to generate/mount certificates, continuing without TLS');
              }
            }
