          ...
          if (config.isDatabase) {
            ...
            if (dockerImage.includes('postgres')) {
              try {
                ...
                // PostgreSQL doesn't support SSL via env vars, so we use ALTER SYSTEM or postgresql.conf
                // Create an init script that enables SSL using ALTER SYSTEM (works at runtime)
                const sslConfigDir = path.join('/tmp', 'postgres-ssl-config', config.name);
                const initScriptPath = path.join(sslConfigDir, 'enable-ssl.sh');
                
                // Ensure directory exists
                if (!fs.existsSync(path.dirname(sslConfigDir))) {
                  fs.mkdirSync(path.dirname(sslConfigDir), { recursive: true });
                }
                if (!fs.existsSync(sslConfigDir)) {
                  fs.mkdirSync(sslConfigDir, { recursive: true });
                }
                
                // Create init script that enables SSL
                // For new containers: Runs during initdb phase, modifies postgresql.conf before first start
                // For existing containers: Can be run manually to enable SSL (requires restart)
                const initScript = `#!/bin/bash
set -e

# This script runs in /docker-entrypoint-initdb.d/ for new containers
# During initdb, postgresql.conf exists but PostgreSQL may not be running yet
# We modify postgresql.conf directly so SSL is enabled when PostgreSQL starts
...
if [ -f /var/lib/postgresql/data/postgresql.conf ]; then
  ...
  if ! grep -q "^ssl = on" /var/lib/postgresql/data/postgresql.conf; then
    ...
    echo "# SSL Configuration (auto-generated by daemon)" >> /var/lib/postgresql/data/postgresql.conf
    echo "ssl = on" >> /var/lib/postgresql/data/postgresql.conf
    echo "ssl_cert_file = '${certMountPath}/${certFileName}'" >> /var/lib/postgresql/data/postgresql.conf
    echo "ssl_key_file = '${certMountPath}/${keyFileName}'" >> /var/lib/postgresql/data/postgresql.conf
  ...
  echo "⚠️  postgresql.conf not found yet (this is normal during early initdb)"
  # Try again after a short wait (postgresql.conf is created during initdb)
  sleep 2
  if [ -f /var/lib/postgresql/data/postgresql.conf ]; then
    echo "" >> /var/lib/postgresql/data/postgresql.conf
    echo "# SSL Configuration (auto-generated by daemon)" >> /var/lib/postgresql/data/postgresql.conf
    ...
    echo "ssl_key_file = '${certMountPath}/${keyFileName}'" >> /var/lib/postgresql/data/postgresql.conf
    echo "ssl_min_protocol_version = 'TLSv1.2'" >> /var/lib/postgresql/data/postgresql.conf
    echo "✅ SSL configuration added to postgresql.conf (after wait)"
  fi
fi

echo "SSL will be enabled when PostgreSQL starts"
`;
                
                fs.writeFileSync(initScriptPath, initScript, 'utf8');
                fs.chmodSync(initScriptPath, '755');
                
                // Mount init script to docker-entrypoint-initdb.d for new containers
                // For existing containers, this will be available for manual execution
                containerConfig.HostConfig.Binds.push(
                  `${initScriptPath}:/docker-entrypoint-initdb.d/99-enable-ssl.sh:ro`
                );
                ...
                logger.info(
                  ...
                  'Mounted SSL certificates and created SSL enable script for PostgreSQL container'
                );
              } catch (error) {
                logger.warn(
        ...
        // Enable SSL for PostgreSQL containers after creation
        // The init script only runs on first initialization, so we need to enable it programmatically
        if (config.isDatabase && dockerImage.includes('postgres') && config.useTraefik && config.domain && config.subdomain) {
          try {
            const fullDomain = `${config.subdomain}.${config.domain}`;
            const certPaths = await this.certificateService.generateCertificate(config.name, fullDomain);
            const certMountPath = '/var/lib/postgresql/ssl';
            ...
            if (containerInfo.State.Status !== 'running') {
              logger.info({ containerId: containerInfo.Id, name: config.name }, 'Starting container to enable SSL');
            ...
            logger.info({ containerId: containerInfo.Id, name: config.name }, 'Waiting for PostgreSQL to be ready');
            let postgresReady = false;
            for (let i = 0; i < 30; i++) {
              try {
                await execCommand(
                  `docker exec ${containerInfo.Id} pg_isready -U postgres`,
                  { stdio: 'ignore' as const, timeout: 2000 }
                );
                postgresReady = true;
                break;
              } catch (error) {
            ...
            if (!postgresReady) {
              logger.warn({ containerId: containerInfo.Id, name: config.name }, 'PostgreSQL not ready after 30 seconds, SSL may not be enabled');
            } else {
              // Enable SSL in postgresql.conf
              logger.info({ containerId: containerInfo.Id, name: config.name }, 'Enabling SSL in PostgreSQL');
              
              // Check if SSL is already enabled
              try {
                const checkResult = await execCommand(
                  `docker exec ${containerInfo.Id} sh -c "grep -q '^ssl = on' /var/lib/postgresql/data/postgresql.conf && echo 'enabled' || echo 'disabled'"`,
                  { timeout: 5000 }
                );
                
                if (checkResult.stdout.trim() === 'enabled') {
                  logger.info({ containerId: containerInfo.Id, name: config.name }, 'SSL already enabled in postgresql.conf');
                } else {
                  // Add SSL configuration line by line to avoid escaping issues
                  const sslLines = [
                    '',
                    '# SSL Configuration (auto-generated by daemon)',
                    'ssl = on',
                    `ssl_cert_file = '${certMountPath}/${certFileName}'`,
                    `ssl_key_file = '${certMountPath}/${keyFileName}'`,
                  ...
                  for (const line of sslLines) {
                    ...
                    await execCommand(
                      `docker exec ${containerInfo.Id} sh -c "echo '${escapedLine}' >> /var/lib/postgresql/data/postgresql.conf"`,
                      { stdio: 'ignore' as const, timeout: 5000 }
                    );
                  }
                  
                  logger.info({ containerId: containerInfo.Id, name: config.name }, 'SSL configuration added to postgresql.conf');
                }
              } catch (error) {
                logger.warn(
                  { containerId: containerInfo.Id, name: config.name, error: this.getErrorMessage(error) },
                  'Failed to enable SSL in postgresql.conf, will try to restart anyway'
                );
              }

              // Restart container so SSL takes effect
              logger.info({ containerId: containerInfo.Id, name: config.name }, 'Restarting container to apply SSL configuration');
              await container.restart();
              // Wait for container to be ready after restart
              await new Promise(resolve => setTimeout(resolve, 3000));
              
              logger.info({ containerId: containerInfo.Id, name: config.name }, 'SSL enabled and container restarted');
            }
          } catch (error) {
            logger.warn(
              { 
              ...
              'Failed to enable SSL on PostgreSQL container, continuing without SSL'
            );
            // Continue without SSL - connection strings will show non-TLS option
          }
        }
  ...
  async removeContainer(containerId: string, removeVolumes: boolean = true): Promise<void> {
    return this.retryOperation(async () => {
      try {
        ...
        let needsForceRemove = false;
        
        try {
          containerInfo = await container.inspect();
          containerState = containerInfo.State.Status || 'unknown';
          
          // Check if container is in an error state that might need force removal
          // "dead" state definitely needs force, "exited" might be normal or error
          const isDeadState = containerState === 'dead';
          const hasError = containerInfo.State.Error && containerInfo.State.Error !== '';
          
          if (isDeadState || hasError) {
            logger.info({ containerId, state: containerState, error: containerInfo.State.Error }, 'Container is in error state, will use force removal if needed');
            needsForceRemove = true;
          }
        } catch (inspectError) {
          // If we can't inspect, the container might not exist
          const errorMsg = this.getErrorMessage(inspectError);
          if (errorMsg.includes('no such container') || errorMsg.includes('not found')) {
            logger.info({ containerId }, 'Container not found, may already be removed');
        ...
        // Try to stop container if running (but don't fail if it doesn't work)
        // Containers in error states might not be stoppable, but can still be removed
        if (containerInfo.State.Running) {
          try {
            await container.stop({ t: 10 });
            logger.debug({ containerId }, 'Container stopped successfully');
          } catch (stopError) {
            const errorMsg = this.getErrorMessage(stopError);
            // Log warning but continue - container might be in error state
            logger.warn({ containerId, error: errorMsg, state: containerState }, 'Could not stop container, will attempt force removal');
            needsForceRemove = true; // Mark for force removal if stop failed
          }
        } else {
          logger.debug({ containerId, state: containerState }, 'Container not running, skipping stop');
        }

        // Remove container (use force if stop failed or container is in error state)
        try {
          await container.remove({ v: false, force: needsForceRemove });
          logger.info({ containerId, force: needsForceRemove }, 'Container removed successfully');
        } catch (removeError) {
          const errorMsg = this.getErrorMessage(removeError);
          // If container not found, that's fine - it's already gone
          if (errorMsg.includes('no such container') || errorMsg.includes('not found')) {
            logger.info({ containerId }, 'Container already removed');
            return;
          }
          // If first attempt failed and we didn't use force, try with force
          if (!needsForceRemove) {
            logger.warn({ containerId }, 'First removal attempt failed, retrying with force');
            try {
              await container.remove({ v: false, force: true });
              logger.info({ containerId }, 'Container removed with force');
            } catch (forceRemoveError) {
              const forceErrorMsg = this.getErrorMessage(forceRemoveError);
              if (forceErrorMsg.includes('no such container') || forceErrorMsg.includes('not found')) {
                logger.info({ containerId }, 'Container already removed');
                return;
              }
              throw forceRemoveError;
            }
          } else {
            throw removeError;
          }
